...
export function proposeConnectionWrite(state, provider, body, actor) {
  ...
  if (provider !== 'shopify' || !['product_content', 'internal_note'].includes(body.operation)) throw connectionError('Runvara does not support this write action. No change was made.', 'WRITE_UNSUPPORTED', 422);
  ...
  if (!product || !/^gid:\/\/shopify\/Product\/\d+$/.test(product.id)) throw connectionError('Choose a Shopify product in this workspace.', 'PRODUCT_NOT_FOUND', 404);
  const input = { productId: product.id, operation: body.operation };
  if (body.operation === 'internal_note') {
  ...
  if (existing) {
    if (existing.digest !== digest(input)) throw connectionError('This request reference was already used for a different change.', 'WRITE_CONFLICT', 409);
    return existing;
  }
  const connection = state.connections?.find(item => item.provider === 'shopify');
  if (!connection?.metadata?.grantedScopes?.includes('write_products')) throw connectionError('Shopify has not granted product write access. Reconnect with write access and test the connection first.', 'WRITE_SCOPE_REQUIRED', 409);
  const requiresApproval = settings.permissionMode !== 'automatic' || body.operation !== 'internal_note';
  ...
  const write = { id: `write_${crypto.randomUUID()}`, requestId: body.requestId, provider, input, digest: digest(input), connectionId: connection.id, account: connection.metadata.shopDomain, requestedBy: actor,
    requiresApproval, status: requiresApproval ? 'pending_approval' : 'ready', createdAt: new Date().toISOString() };
  if (requiresApproval) {
    const approval = normalizeApprovalRequest({ type: body.operation === 'product_content' ? 'customer_facing_publish' : 'integration_change',
      action: body.operation === 'product_content' ? `Update Shopify content: ${product.title}` : `Save internal Shopify note: ${product.title}`,
      reason: body.operation === 'product_content' ? `Proposed title: ${input.title}\nDescription: ${input.description}`.slice(0, 1000) : input.note,
      expectedBenefit: 'Apply the exact change reviewed in the Connection Centre.', risk: 'medium', source: 'connection-centre',
      evidence: [{ type: 'product', id: product.id, detail: 'The full proposed change is available in the channel details panel.' }],
...
export async function executeConnectionWrite(state, writeId, actor, integrations, persistClaim) {
  ...
  if (['executing', 'uncertain', 'failed'].includes(write.status)) throw connectionError('This change already ran or its result needs checking in Shopify. It will not be sent again.', 'WRITE_ALREADY_ATTEMPTED', 409);
  ...
  if (!connection || connection.metadata?.shopDomain !== write.account) throw connectionError('The store connection changed. Create a new request.', 'WRITE_CONNECTION_CHANGED', 409);
  if (!connection.metadata?.grantedScopes?.includes('write_products')) throw connectionError('Shopify write access is missing.', 'WRITE_SCOPE_REQUIRED', 409);
  ...
  try {
    let result;
    if (write.input.operation === 'internal_note') {
    ...
    if (approval) {
      ...
      recordWork(state, { id: approval.id, title: approval.action, source: 'approval-centre', status: 'COMPLETED', executedExternally: true, evidence: [{ type: 'shopify_write', id: write.input.productId, detail: write.input.operation }] });
    }
    recordWork(state, { id: write.id, title: 'Shopify change confirmed', source: 'connection-centre', status: 'COMPLETED', executedExternally: true, evidence: [{ type: 'shopify_write', id: write.input.productId, detail: write.input.operation }] });
  } catch (error) {
    if (write.status !== 'failed') write.status = 'uncertain';
