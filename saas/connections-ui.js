(() => {
  ...
  window.RunvaraConnections = {
    init(config) {
      api=config;
      $('#connection-journey')?.addEventListener('submit',event=>{if(event.target.id==='journey-platforms'){event.preventDefault();saveJourney({platforms:new FormData(event.target).getAll('platforms')});}});
      $('#connection-journey')?.addEventListener('click',event=>{const target=event.target.closest('button');if(!target)return;if(target.dataset.journeyReview)saveJourney({reviewPermissions:target.dataset.journeyReview});else if(target.id==='journey-finish')saveJourney({finish:true});else if(target.id==='journey-dashboard')api.setView('overview');else if(target.dataset.connectionAction)handleAction(event);});
