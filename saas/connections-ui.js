(() => {
  ...
  function journey() {
    ...
    root.innerHTML = `<details ${returning ? '' : 'open'}><summary>${progress.completedAt ? 'Business setup complete · review setup' : 'Set up your business'}</summary><p class="eyebrow">BUSINESS SETUP</p><h2>${progress.completedAt ? 'Your business is ready' : 'Let’s get your business connected'}</h2><p>Your progress is saved in this workspace. You can leave and return at any time. Select only the platforms you want now; add others later.</p><progress aria-label="Platforms ready" value="${completed}" max="${Math.max(1,progress.platforms.length)}"></progress><p>${completed} of ${progress.platforms.length} selected platforms ready</p><form id="journey-business" class="record-form"><label>Business name<input name="businessName" value="${esc(data.workspace?.name || '')}" minlength="2" maxlength="120" required></label><button class="secondary" ${!owner()?'disabled':''}>Save business details</button></form><form id="journey-platforms"><fieldset ${!owner() ? 'disabled' : ''}><legend>Choose your platforms</legend>${channels.map(channel=>`<label class="check-label"><input type="checkbox" name="platforms" value="${esc(channel.id)}" ${progress.platforms.some(item=>item.provider===channel.id)?'checked':''}>${esc(channel.name)}</label>`).join('')}</fieldset><button class="secondary" ${!owner()?'disabled':''}>Save platform choices</button></form>${progress.platforms.map(item=>{const channel=channels.find(c=>c.id===item.provider);if(!channel)return '';return `<section class="connection-section"><h3>${esc(channel.name)}</h3><ol><li>${item.connected?'Connected':'Connect your account'}</li><li>${item.tested?'Connection tested':'Test the connection'}</li><li>${item.imported?'Initial import complete':'Import your first data'}</li><li>${item.reviewed?'Permissions reviewed':'Review permitted actions'}: ${esc(label(item.permissionMode))}</li></ol>${item.needsAttention?`<p class="warn">${item.blocksSetup === false ? 'Order setup is ready. eBay marketing eligibility still needs review in the connection panel.' : 'This connection needs attention. Open it for the next step.'}</p>`:''}<div class="button-row">${button('open','Manage connection',channel)}<button class="secondary" data-journey-review="${esc(item.provider)}" ${!owner()?'disabled':''}>Keep current permissions</button></div></section>`}).join('')}<p>Read-only is a valid choice. Changing write permissions requires explicit owner consent in the connection panel. Financial and customer-facing changes still require approval.</p><p>Autopilot is ${data.autopilot?.enabled ? 'on' : 'off'}. You control automatic read schedules in Automations. External changes remain permission and approval protected.</p><button class="secondary" id="journey-controls">Review automation settings</button><button class="primary" id="journey-finish" ${!progress.complete||!owner()?'disabled':''}>Finish setup</button><button class="secondary" id="journey-dashboard">View business dashboard</button><p id="journey-feedback" role="status"></p></details>`;
  }
  ...
  function cards() {
    ...
    $('#connection-grid').innerHTML = channels.map(channel=>{
      const repair=channel.recovery?.action==='reconnect';
      const countText=Object.entries(channel.counts || {}).filter(([,value])=>typeof value==='number').map(([key,value])=>`${value} ${label(key)}`).join(' · ');
      const action=repair?'reconnect':channel.configured?'open':'connect';
      return `<article class="connection-card" data-channel-card="${channel.id}"><div class="connection-card-heading"><span class="connection-brand">${ui.logo(channel.id)}</span>${ui.badge(repair?'Access needed':label(channel.status),statusClass(channel.status))}</div><h3><button type="button" class="connection-title" data-connection-action="open" data-provider="${channel.id}" aria-haspopup="dialog">${esc(channel.name)}</button></h3><p class="connection-summary">${esc(channel.identity || (channel.configured?'Account connected':'No account connected'))}</p><p class="connection-health ${channel.recovery?'warn':'muted'}">${esc(ui.connectionMessage(channel))}</p><dl class="connection-facts"><div><dt>Authentication</dt><dd>${esc(repair?'Renew access':channel.lastCheckedAt?'Checked '+when(channel.lastCheckedAt):'Not yet verified')}</dd></div><div><dt>Last successful sync</dt><dd>${esc(when(channel.lastSuccessfulSyncAt))}</dd></div><div><dt>Next automatic read</dt><dd>${esc(ui.schedule(channel,data))}</dd></div><div><dt>Permission policy</dt><dd>${esc(label(channel.settings.permissionMode))}</dd></div></dl><div class="connection-coverage"><span class="eyebrow">SELECTED DATA</span><div class="capabilities">${(channel.settings.areas || []).map(area=>`<span>${esc(label(area))}</span>`).join('') || '<span>No areas selected</span>'}</div><small>${esc(countText || 'No imported coverage reported')}</small></div><details class="evidence"><summary>Capabilities &amp; access</summary><p>Supported reads: ${esc(channel.areas.map(label).join(', '))}.</p><p>${esc(channel.coverageNote || 'Availability depends on provider access and your selected sync areas.')}</p><p>${esc(channel.writeAccessGranted?'Provider write access granted. Runvara’s permission policy and approvals still apply.':'Provider write access has not been confirmed.')}</p><p>Automatic reads follow your configured Autopilot limits.</p></details><div class="connection-card-footer"><div class="button-row">${button(action,repair?`Reconnect ${channel.name}`:channel.configured?'Manage connection':`Connect ${channel.name}`,channel,'primary')}${channel.configured?button(repair?'open':'sync',repair?'Review access':'Sync now',channel):''}</div></div></article>`;
    }).join('');
  }
  ...
  function renderPanel() {
    ...
    $('#connection-detail').innerHTML = `<div class="section-head stack-mobile"><div><p class="eyebrow">CONNECTION CENTRE</p><h2 id="connection-dialog-title">${esc(channel.name)}</h2><p class="muted">${esc(channel.identity || 'No account connected')}</p></div><span class="tag ${statusClass(channel.status)}">${esc(label(channel.status))}</span></div>
      ...
      ${channel.recovery ? `<div class="connection-notice"><p>${esc(channel.recovery.message)}</p>${channel.recovery.action === 'manager' ? (data.workspace?.id === 'packsmart-solutions' ? customerZeroManagerLink : '<p>Review older listings in your eBay Seller Hub.</p>') : button(channel.recovery.action, channel.recovery.label, channel, 'primary')}</div>` : ''}
      ...
      ${channel.configured ? diagnostics(channel) : ''}
  ...
  window.RunvaraConnections = {
    init(config) {
      api=config;
      $('#connection-journey')?.addEventListener('submit',event=>{if(event.target.id==='journey-business'){event.preventDefault();saveJourney({businessName:new FormData(event.target).get('businessName')});}else if(event.target.id==='journey-platforms'){event.preventDefault();saveJourney({platforms:new FormData(event.target).getAll('platforms')});}});
      $('#connection-journey')?.addEventListener('click',event=>{const target=event.target.closest('button');if(!target)return;if(target.dataset.journeyReview)saveJourney({reviewPermissions:target.dataset.journeyReview});else if(target.id==='journey-finish')saveJourney({finish:true,reviewControls:true});else if(target.id==='journey-controls')api.setView('automations');else if(target.id==='journey-dashboard')api.setView('overview');else if(target.dataset.connectionAction)handleAction(event);});
